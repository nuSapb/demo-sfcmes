const { getAllRoles, getRoleById: getRoleByIdQuery, createRole: createRoleQuery, updateRoleById, deleteRoleById } = require('../queries/roleQueries');
...
const getRoleById = async (req, res) => {
    const { id } = req.params;
    try {
        const role = await getRoleByIdQuery(id);
        if (!role) {
...
const createRole = async (req, res) => {
    ...
    try {
        const newRole = await createRoleQuery(roleData);
